			...
			success: (data) => {
				...
				data.find('.giveaway__row-outer-wrap').each((index, item) => {
					...
					if(
						...
						( this.getConfig('ending', 0) === 0 || GA.left <= this.getConfig('ending', 0) ) && // Время до окончания
						( this.getConfig('min_chance', 0) === 0 || GA.chance >= this.getConfig('min_chance', 0) ) // Минимальный шанс
					)
	...
	giveawaysEnter(wishlist, callback){
		...
		function processOne(){
			...
			if(
				...
				( ignoreForWishlist || _this.getConfig('min_level') === 0 || GA.level >= _this.getConfig('min_level') ) && // Минимальный уровень
				( ignoreForWishlist || GA.cost >= _this.getConfig('min_cost') ) && // Минимальная стоимость
				( ignoreForWishlist || _this.getConfig('max_cost') === 0 || GA.cost <= _this.getConfig('max_cost') ) && // Максимальная стоимость
				( ( wishlist && _this.getConfig('reserve_on_wish') ) || _this.getConfig('points_reserve') === 0 || ( (_this.curr_value - GA.cost) >= _this.getConfig('points_reserve') ) ) // Резерв очков
			)
			{
				$.ajax({
