...
class TF2R extends Seeker {
  ...
  authCheck(callback) {
    ...
    this.ajaxReq(this.websiteUrl, response => {
      if (response.success) {
        if (response.data.indexOf(authContent) >= 0) callback(1);
        else callback(0);
      } else callback(-1);
    });
  }
  ...
  seekService() {
    ...
    _this.ajaxReq("http://tf2r.com/raffles.html", response => {
      ...
      function giveawayEnter() {
        if (giveaways.length <= curr_giveaway || !_this.started) return;
        ...
        _this.ajaxReq(link, response => {
          if (response.success) {
            ...
            if (entered || response.data.indexOf("Fuck off") >= 0) return;
